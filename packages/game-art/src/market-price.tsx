...
export interface MarketPriceLabels {
  ...
  /** Hover text: which of the two bases this figure is, and how old the quote behind it is. */
...
export function MarketPrice({
  ...
  const title = priced ? labels.title(price) : undefined;
  ...
  return (
    <span className={cn('inline-flex items-center gap-1 text-xs tabular-nums', className)}>
      {price.listingUrl == null ? (
        <span className="inline-flex items-center gap-1" title={title}>
          {content}
        </span>
      ) : (
        <a
          href={price.listingUrl}
          target="_blank"
