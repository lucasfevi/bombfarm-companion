 ...
 * prerequisite by type, and the "Let the app forge, equip and reset points" switch is a
 * prerequisite by the only constructor — two independent mechanisms, neither of which anyone has
 * to remember.
 */
...
export class WriteNotEnabledError extends Error {
  constructor() {
    super('WriteNotEnabledError: account writes are off in Settings ("Let the app forge, equip and reset points")');
    this.name = 'WriteNotEnabledError';
  }
}

/** Thrown at runtime when `write-request.ts` is handed a value typed as `WriteSession` that was
...
export class WriteSessionRequiredError extends Error {
  constructor() {
    super('WriteSessionRequiredError: requestPost/buildWriteRequest require a session minted by grantWriteSession');
