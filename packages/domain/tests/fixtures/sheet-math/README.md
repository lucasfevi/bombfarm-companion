...
| May prove | **a House that BINDS** — 5 recovery slots against 7 heroes, a regime no other committed capture holds (486's captures all run `casa.slots` at or above their roster size), so the team-plan and House-allocation suites finally have a real contended roster instead of a `slots` override; **`golpe_brutal`** on a real hero (Ivo L51, 20/20) — the corpus's first, closing the gap `points-within-level-budget.test.ts` layer 2 was written to work around; **`fortuna`** on two real heroes (Ivo 20/20, Gale 8/20), which the gold-multiplier suites previously had to synthesise with `withAbilityLevels`; a **five-geared / two-naked** split on one roster; and a whole-roster exact solve — all 7 heroes recover **exactly `level`** points with zero inference issues |
...
| May **not** prove | star scaling of either flat sheet-ability addend (every hero is ★0); Presságio Mortal (no hero owns it); the per-POINT crit-chance/CDR rates (no hero here holds a crit-chance or cooldown point); a House that binds (`casa.slots: 9` comfortably covers 11 heroes with the field at 9 — see `save-20260819-11882-7heroes.json` for the contended case); before/after point deltas (`stat_points_available` is `0` on every hero) |
