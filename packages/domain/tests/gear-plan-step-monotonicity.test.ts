...
import type { GearPlanInput, ScopeState } from '@bombfarm/domain/gear-plan/types';
 ...
 * untested: `gear-plan-waterfall-requires-full-plan.test.ts` exercises it directly with a mocked
 ...
 */
function assertStepInvariants(result: ReturnType<typeof runGearPlan>, input: GearPlanInput): void {
...
function runAtBudget(input: GearPlanInput, key: string) {
  const maxEvaluations = FULL_BUDGET_CASES.has(key) ? undefined : REDUCED_MAX_EVALUATIONS;
  return runGearPlan(input, maxEvaluations === undefined ? undefined : { maxEvaluations });
}

describe('gear plan step monotonicity (roster level)', () => {
  ...
  it(
    ...
    () => {
      const input = gearPlanInputFromFixture('save-20260801-crit-dmg-tree.json', 10);
      input.account.slots = 3;
      input.scopeByHeroId = donateMix(input.heroes.map((h) => h.heroId));
      assertStepInvariants(runGearPlan(input, { maxEvaluations: REDUCED_MAX_EVALUATIONS }), input);
    },
    GRID_TIMEOUT_MS,
  );
  ...
  it(
    ...
    () => {
      const input = gearPlanInputFromFixture('save-20260801-crit-dmg-tree.json', 20);
  ...
  it(
    ...
    () => {
      // This config (also used at full budget in gear-plan-waterfall.test.ts) reliably produces
      ...
      const result = runGearPlan(input, { maxEvaluations: REDUCED_MAX_EVALUATIONS });
      if (result.blocked) throw new Error('plan blocked');
