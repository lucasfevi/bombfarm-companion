 ...
 *
 * The fixture supplies a realistic loadout/sheetOther baseline only; the levels asserted against
 * are read off the fixture hero rather than pinned, so no hero identity or captured value is
 * asserted here. Every hero is zeroed before the passes run, so the one hero who banks her whole
 * budget unspent is no different from the rest: the pool is `level` either way.
 */
