 ...
 * 2. **The Golpe Brutal case** — a constructed hero with a SATURATED point budget, carrying the
 *    ability at 20/20 and the two `crit_dmg` numbers the game itself reported for the real Ivo.
 *    Written when no committed capture carried the ability at all; `save-20260819-11882-7heroes.json`
 *    now does (Ivo again, four days later at L51, still 20/20), so layer 1 exercises the bug on a
 *    real roster. Layer 2 is kept because it does something layer 1 cannot: it saturates the
 *    budget deliberately, so an invented crit-damage point has nowhere to hide but over the
 *    ceiling. The values are the game's own reading, not this model's output (`AD-068`) — the same
 *    practice `save-units.test.ts` uses for Bellatrix's `crit_dmg` literal.
 */
