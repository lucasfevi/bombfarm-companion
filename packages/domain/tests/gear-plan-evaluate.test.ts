...
import {
  ...
  evaluateRoster,
} from '@bombfarm/domain/gear-plan/evaluate';
import { computeRosterAuras } from '@bombfarm/domain/gear-plan/auras';
import { scoreHeroLoadout } from '@bombfarm/domain/gear-plan/score';
import {
  buildHeroPlanContexts,
} from '@bombfarm/domain/gear-plan/hero-context';
import type {
  EvaluateRosterInput,
  FarmContext,
  HeroPlanContext,
} from '@bombfarm/domain/gear-plan/types';
import {
  extractHero,
  loadFixtureJson,
  treeTotalsFromSave,
...
function fixtureEvaluation(file: string): EvaluateRosterInput {
  ...
  const heroes = (raw.heroes as unknown[])
    ...
    .filter((h): h is GearPlanHeroInput => h != null);
