...
describe('catalog completeness (T2)', () => {
  ...
  it('save slot === ABILITIES.findIndex(id) + 1 for every owned code in the fixture', () => {
    // F1 (ground-truth-rule class (b) — structural): re-pointed onto payload-20260812-8heroes.json.
...
describe('rank-20 migration', () => {
  ...
  // Every row cites its source in the wiki ability table; perLevel x 20 reproduces the wiki's
  ...
  const MID_CURVE_ABILITIES: Array<{
    ...
    { id: 'bateria_extra', perLevel: 1, wikiTotalAtCap: 20, citation: 'wiki Grimório ability table' },
    { id: 'marcha_acelerada', perLevel: 0.185, wikiTotalAtCap: 3.7, citation: 'wiki Grimório ability table' },
    ...
    { id: 'detonacao_dupla', perLevel: 1.5, wikiTotalAtCap: 30, citation: 'wiki Grimório ability table' },
    { id: 'folego_mineiro', perLevel: 1, wikiTotalAtCap: 20, citation: 'wiki Grimório ability table' },
    { id: 'grito_guerra', perLevel: 1, wikiTotalAtCap: 20, citation: 'wiki Grimório ability table' },
  ];

  it.each(MID_CURVE_ABILITIES)(
    '$id perLevel is $perLevel and x20 equals the wiki total at cap ($wikiTotalAtCap), per $citation',
  ...
  it('rank 20 and a mid-curve rank both survive unclamped through abilityMods', () => {
    // explosao_ampla @20 exists in the fixture; this checks the pure catalog math handles it
    // without clamping.
...
describe('sheetOther.critDmgFlat wiring — all four production builders', () => {
  ...
  // Wiring it anyway is still correct (this file's literal requirement, and defensive against a
