...
describe('guard (c) — no retired one-shot identifier survives, with a considered allowlist', () => {
  ...
  const PERMANENT_ALLOWLIST: Allowed[] = [
    ...
    // from 8 word-bounded `oneshot` matches to 0, so the entry permitted something that cannot
    // occur and is removed rather than left to widen this list beyond what is real.
    {
      file: join(WEB_SRC_ROOT, 'shared/lib/account-shared.ts'),
      reason: "normalizeRankMode's own doc comment names the retired 'oneshot' value it migrates away from",
    },
    {
