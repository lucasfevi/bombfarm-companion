 ...
 *
 * The web app enforces this over its own `src` and cannot see this package, so the optimizer
 * screen's components would have left the budget's reach the moment they moved here. This is
 * that guard, mirrored — same limit, same counting rule, same empty allowlist — so the rule
 * follows its subjects instead of quietly ceasing to apply to them.
 *
...
`;
  ...
  it('no component in @bombfarm/team-plan exceeds 8 own props', () => {
