 ...
 *
 * The web app enforces this over its own `src` and cannot see this package; `@bombfarm/farm` then
 * mirrored the guard when the farm screen moved there, and it scopes its "the scan reached
 * something" anchor to `farm-*` filenames — so the hero and roster views left the budget's reach a
 * second time when they moved here. This is that guard, mirrored again — same limit, same counting
 * rule, same empty allowlist — so the rule follows its subjects instead of quietly ceasing to
 * apply to them.
 *
...
describe('max props (error, non-DOM props only, no allowlist)', () => {
  ...
  it('the scan reaches this package\'s components — it is not passing over an empty set', () => {
    ...
    expect(names).toContain('hero-picker-table.tsx');
    expect(names).toContain('phases-hero-panel.tsx');
  ...
  sort: { key: RosterSortKey; direction: RosterSortDir };
...
`;
  ...
  it('no component in @bombfarm/hero exceeds 8 own props', () => {
