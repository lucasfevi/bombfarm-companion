/**
 * The main-loop half: the strongest available structural form of "a recompute does not block the
 * Electron main event loop long enough to delay IPC" is that main never calls the functions that
 * would block it at all. `pipelineForHero`, `computeAdvisorPipeline` and `parseAccountPayload`
 * appearing zero times under `apps/desktop/src/main/**` is what asserts the
 * renderer-recomputes, main-detects-changes placement decision.
 *
...
/** Same two exclusions the repo's other identifier-scan guards already established for this
 *  genre: test files legitimately NAME these identifiers to assert their own absence, and doc
 *  comments legitimately explain *why* a function is never called here. Both are text about the
