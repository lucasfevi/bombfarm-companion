...
import { liveHeroPeeks } from './use-live-hero-peeks';
...
describe('liveHeroPeeks', () => {
  it('keys one card per roster hero by the game id a live row carries', () => {
    const peeks = liveHeroPeeks(rosterOf(record('h1', 'Vex', 42), record('h2', 'Nim', 7)));
  ...
  it('carries the sheet, the abilities and the loadout — what the row itself cannot say', () => {
    const peek = liveHeroPeeks(rosterOf(record('h1', 'Vex', 42))).get('h1');
  ...
  it('answers nothing for a hero the roster does not hold', () => {
    expect(liveHeroPeeks(rosterOf(record('h1', 'Vex', 42))).get('h9')).toBeUndefined();
  });

  it('has no cards at all when the account did not parse', () => {
    expect(liveHeroPeeks(null).size).toBe(0);
    expect(liveHeroPeeks(null).get('h1')).toBeUndefined();
  });
});
