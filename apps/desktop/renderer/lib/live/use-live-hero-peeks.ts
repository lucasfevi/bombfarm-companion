...
/**
 * One card per roster hero, keyed by the game's own hero id — the same id a live row carries, so
 * a live row finds its card by id alone. Built once per roster, not per row and not per tick:
 * the rows are memoised on their props, and a card rebuilt on every fast-channel tick would
 * re-render every row four times a second.
 */
export function liveHeroPeeks(roster: AccountRoster | null): ReadonlyMap<string, HeroPeekData> {
...
export function useLiveHeroPeeks(): (heroId: string) => HeroPeekData | undefined {
  ...
  const peeks = useMemo(() => liveHeroPeeks(roster), [roster]);
