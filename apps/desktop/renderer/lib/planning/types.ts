...
/**
 * `AD-036`'s resolution of the MPV-01/MPV-11 contradiction: a computed verdict, distinct from
 * `AccountFidelityReport.grade` (which is `unavailable` after every restart on a healthy store —
 * §2.1 of design.md). See design.md §3 `AD-036`'s table for the condition ↔ render mapping.
 */
...
export type RosterEntry = {
  ...
  /** `ImportCandidate.blocked` — withholds this hero's `dps`/`nextPointRanking`/`resetAdvice` only (MPV-10). */
  readonly blocked: boolean;
};

export type PlanningModel = {
  readonly availability: Availability;
  /** From `@bombfarm/domain`'s `deriveAccountFidelity`, unchanged — the provenance display (MPV-06/07). */
  readonly report: AccountFidelityReport;
  /** `ACCOUNT_SECTIONS` order (MPV-07). */
...
/** The five things the pipeline can produce, each gated by its own dependency set (`AD-041`). */
