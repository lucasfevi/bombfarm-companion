...
describe('HeroCard', () => {
  it('carries the live-hero-card-<id> testid keyed by the hero id', () => {
    const html = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7', name: 'Astra', grade: 'A' } }));
    expect(html).toContain('data-testid="live-hero-card-hero-7"');
  });
  ...
  it('withholds the rank letter when the name is missing — a grade without a name is half a roster join', () => {
    const html = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7', grade: 'A' } }));
    expect(html).toContain('data-testid="live-hero-card-hero-7-name"');
  ...
  it('renders the trailing content passed in, such as a countdown', () => {
    const html = renderToStaticMarkup(
      createElement(HeroCard, { hero: { id: 'hero-7' }, trailing: createElement('span', { 'data-testid': 'trailing-probe' }, 'x') }),
    );
  ...
  it('an id-only hero (no name, grade, rarity, stars or level) renders without throwing, falling back to the id for the name and an em dash for the rank', () => {
    const html = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7' } }));
    expect(html).toContain('data-testid="live-hero-card-hero-7"');
    expect(html).toContain('data-testid="live-hero-card-hero-7-name"');
    expect(html).toContain('hero-7');
  ...
  it('renders the rank letter in accent when present, and an em dash in muted when absent', () => {
    const withGrade = renderToStaticMarkup(
      createElement(HeroCard, { hero: { id: 'hero-7', name: 'Astra', grade: 'S' } }),
    );
    const withoutGrade = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7', name: 'Astra' } }));
  ...
  it('renders the hero name in sentence case, never uppercased by class or markup', () => {
    const html = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7', name: 'Aurora' } }));
    expect(html).toContain('>Aurora<');
    expect(html).not.toContain('uppercase');
  });

  it('renders stars as repeated glyphs only when stars is present and greater than zero', () => {
    const withStars = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7', stars: 2 } }));
    const zeroStars = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7', stars: 0 } }));
    const noStars = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7' } }));
  ...
  it('renders the rarity label, coloured by rarity, only when rarity is present', () => {
    const withRarity = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7', rarity: 3 } }));
    const noRarity = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7' } }));
  ...
  it('renders distinct portraits for heroes with distinct joined skins, not the same placeholder for every hero', () => {
    const first = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-a', skin: 2 } }));
    const second = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-b', skin: 6 } }));
...
describe('HeroCard — the muted treatment', () => {
  it('marks a muted card with data-muted, and leaves an ordinary one unmarked', () => {
    const muted = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7' }, muted: true }));
    const plain = renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7' } }));
    expect(muted).toContain('data-muted=""');
    expect(plain).not.toContain('data-muted');
  });

  it('drains colour by attribute alone, so muting never changes the card box and never reflows a section', () => {
    const stripMarker = (html: string) => html.replace(' data-muted=""', '');
    expect(stripMarker(renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7' }, muted: true })))).toBe(
      renderToStaticMarkup(createElement(HeroCard, { hero: { id: 'hero-7' } })),
    );
  });
});
