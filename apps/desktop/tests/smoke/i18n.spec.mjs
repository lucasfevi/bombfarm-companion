...
/** The Live/Farm/Heroes/Inventory/Forge/Optimizer/Account/Settings nav buttons live in AppShell's persistent sidebar
  ...
  test('OS-detected PT-BR, live switch to English with no reload, zero account:changed, no layout shift, and English survives a restart', async () => {
    ...
    try {
      ...
      try {
        ...
        await expectNavWord(page1, 6, pt('accountNavLabel'));
        await expectNavWord(page1, 7, pt('settingsNavLabel'));
        ...
        await expectNavWord(page1, 6, en('accountNavLabel'));
        await expectNavWord(page1, 7, en('settingsNavLabel'));
        ...
        // --- No recompute on a language change (e2e half): the switch fired zero account:changed events --------------
      ...
      try {
        ...
        // The new tab's word must survive a relaunch too, not just the live switch above.
