...
- Updated dependencies [5b64748]
  ...
  - @bombfarm/ui@0.16.0
  - @bombfarm/game-api@0.6.2
  - @bombfarm/account@0.3.5
  - @bombfarm/game-data@0.0.19

## 0.21.0

### Minor Changes

- aa322d8: The Optimizer scores for the same three things the Skill Tree does: gold per hour, a gate clear, or the PVP duel. The rotation "DPS" option is gone — a rotation rewards a stint that outlasts the rest, so it recommended energy to a player whose problem was a gate timer.
...
- Updated dependencies [1ef139c]
  ...
  - @bombfarm/pricing@0.3.0
  - @bombfarm/ui@0.13.1
  - @bombfarm/domain@1.1.1
  - @bombfarm/game-api@0.4.4
  - @bombfarm/game-data@0.0.15
  - @bombfarm/account@0.2.5
  - @bombfarm/farm@1.1.1
  - @bombfarm/game-art@0.5.1
  - @bombfarm/hero@0.2.1
  - @bombfarm/team-plan@0.1.1
...
- 306d2d0: Say which phase a hero's combat numbers are for, and let you ask about another one.
...
- 306d2d0: Fill in the Heroes screen.

  **Your whole roster down one side, one hero's detail beside it.** The tab that has been showing an
  empty state since it arrived now draws the account you already have: every hero the companion read,
  listed best birth roll first, with that hero's identity, birth roll and abilities beside the list.
  Below the width the two panes stop fitting side by side, the list steps aside and is reached
  through the roster picker instead, so the detail keeps the whole window.

  **A hero stays selected because it is that hero, not because it is fourth in the list.** The
  account is re-read every few seconds and the list can come back longer, shorter or in a different
  order. The screen holds on to which hero you were looking at, and only moves when that hero is
  genuinely no longer on the account — then it falls back to the top of the list rather than to
  whatever row took its place.

  **Two different silences, said differently.** "Nothing has been read from your account yet" and
  "this account has no heroes" are separate answers now. A roster that is empty because the game has
  never been open is not the same fact as an account that genuinely owns nobody, and the screen no
  longer tells the first player they own nothing.

  **Ties in roll quality no longer make rows jump.** Whole grades of heroes land on exactly the same
  roll-quality number, and the list orders those by a fixed rule instead of by the order the account
  read happened to arrive in — so re-reading your account does not reshuffle the middle of the list.
...
- b0f4431: See the whole roster at once, as a board of cards.
...
- 23e9626: Stop `pnpm dev` printing Next's "Cross origin request detected from 127.0.0.1" warning on every
...
- c064321: Send a version-stamped `User-Agent` on account and forge requests. `node:https` sends no
...
- 374c22d: A hero whose spent stat points the app could not read from the account is left out of the
  Optimizer's search — not on the scope board, not in the plan — and named above the screen,
  instead of being planned as if it had none spent.
...
- e656239: Recover on their own from a failed hook discovery, and say in the log why one failed.
...
- a6dba79: Add a support link to the desktop app, in the two shapes the web planner already uses: an
  icon-only pill in the top bar beside the language toggle, and a labelled button in a new
  Settings section. Both open the page in the default browser rather than inside the app.

### Patch Changes

- 4b6d4ba: On Windows, closing the companion window now hides it to the system tray instead of quitting. Live
  tracking keeps running while the window is hidden. Right-click the tray icon for **Show** and
  **Quit**; left-click or double-click the icon also brings the window back.

  The main window can be pinned always-on-top from Settings, and it remembers where you left it —
  size, position, and maximized state — across restarts on the same display.
...
- Updated dependencies [652ab4a]
  ...
  - @bombfarm/game-api@0.3.6
  - @bombfarm/game-data@0.0.12
