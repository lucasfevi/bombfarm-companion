...
1. **`sheetOther` is mandatory** on both `applyGear` calls (on-sheet ability contributions such as Ponta / Olho). Dropping it is a fail — tests should spy/arg-assert call sites when output equality cannot discriminate.
