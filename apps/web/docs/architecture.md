...
**Status:** hard truth
**Cursor stub:** [`.cursor/rules/architecture.mdc`](../.cursor/rules/architecture.mdc)
**Sources:** modular-architecture programme W1–W7 (`app/` + `features/` + `shared/` layered tree, lint-enforced boundaries, Zustand root store); app shell ADR [`adr/013-app-shell-route-group.md`](adr/013-app-shell-route-group.md); performance rules in [`react-performance.md`](react-performance.md); design-system layout in [`design-system.md`](design-system.md); state rules in [`state-management.md`](state-management.md) (draft); naming rules in [`naming.md`](naming.md) (draft)
...
| --- | --- | --- |
| `design-system/` | Presentational primitives with no game-domain knowledge | Anything importing `shared/domain` or a feature |
| `game-art/` | Game-aware presentational components — icons, art frames, gold-value display that couple game type data (rarity, item type) with rendering | Generic (non-game) primitives → `design-system/`; game math → `domain/` |
...
| `domain/` | Pure functions over game types, zero React, unit-testable in isolation | Anything importing React or `shared/stores` |
| `i18n/` | User-facing strings, split by namespace | Game term maps stay in `domain/game-labels` ([`i18n.md`](i18n.md)) |
...
**Selectors are the public read API** (`src/shared/stores/selectors/`, one file per slice-family plus `advisor-selectors.ts` and `tab-status-selectors.ts`). A bare `usePlannerStore()` call with no selector is forbidden (subscribes to everything); every read goes through a named selector or an inline field selector. Selectors returning objects or arrays use `useShallow` — except **`selectAdvisorPipeline`**, which is a module-level, single-entry **memoized selector** over a 25-member dependency tuple that already returns a stable object identity on cache hits; wrapping it in `useShallow` would shallow-compare ~40 fields on every store write and defeat the memoization. Full rules: [`state-management.md`](state-management.md) (draft — not yet a hard truth, see that doc's status line).

Persistence is `localStorage` only, via `src/shared/lib/storage.ts`, driven by explicit `subscribeWithSelector` subscriptions in `src/shared/stores/persistence/` — **not** `zustand/persist` (confirmed zero matches for `zustand/persist` in `src/`, [`../.specs/features/modular-architecture-w8-measure-close/bundle-delta.md`](../.specs/features/modular-architecture-w8-measure-close/bundle-delta.md)). No game-server / Electron / memory readers exist in this repo. Public-save compatibility: [`local-data-compat.md`](local-data-compat.md).

**Residual finding (recorded, not fixed here — out of scope for a docs-only wave):** the `devtools` production guard does not actually tree-shake. `planner-store.ts` imports `devtools` unconditionally at module top and only skips *calling* it under `NODE_ENV === 'production'`; webpack's tree-shaking decides module inclusion from the unminified import graph before Terser folds that now-constant branch, so the whole `devtools` module still ships to production (confirmed present in the shipped bundle — see the bundle-delta doc linked above). Byte cost is small (≈1.5 kB gzip) and does not blow any bundle budget, but the guard should eventually become a real conditional import.

## MOD-05 — package-extraction path (documented, not implemented)

The programme decided **against** a pnpm workspace / monorepo split now, but structured `shared/design-system/` and `shared/stores/` so a later `packages/` extraction is mechanical: each already has an `index.ts` public API and no deep imports reach into it from outside (enforced by `boundaries/entry-point`, above). If a `packages/` split ever happens, these two folders move with **no import rewrites outside their own folder**. This is a documented path, not a task in flight — no `packages/` directory exists today.

## Ownership rules

1. Colocate UI state in the leaf/shell that owns it — not the root composer (see [`react-performance.md`](react-performance.md)).
2. Pure math stays in `src/shared/domain/` (`model/`, `gear/`, `stat-breakdown/`, `derive.ts`, `advisor-pipeline.ts`, `phases.ts`, `phase-wiki.ts`, `phase-intel.ts`, `game-labels.ts`, …). Wiki phase rows ship as `src/shared/domain/data/phase-wiki.json` (sync via `pnpm sync:wiki-phases`). React stays thin. When that math (or the panels it names) changes, keep the explain tab in sync — see [`explain-math.md`](explain-math.md).
...
6. Per-tab soft/warn chrome is pure logic in [`src/shared/domain/planner-tab-status.ts`](../src/shared/domain/planner-tab-status.ts) (`computePlannerTabStatuses`); UI shows a **dot + DS Tooltip** for the active hero's own trust/setup state — never an in-flow banner for that ([`animation.md`](animation.md) rule 8 carves out one narrow, reviewed exception for roster-scoped advice about other heroes; it does not apply to per-tab chrome). Matrix: **Hero** soft = default sheet and/or unspent ability points; **Gear** soft = no items — no warn tier (Stats moved to Points and is read-only there, so Gear no longer flags a sheet mismatch); **Account** never badges (skill tree ×1 is a valid default); **Points** soft = missing gear/sheet and/or unspent points (`level − spent`), warn = the Tier-1 reset gate recommends a points reset (`resetAdviceRecommend`, ≥1% sustained DPS gain — see [`explain-math.md`](explain-math.md)/`reset-advice.ts`).
...
9. Compound design-system primitives (`Dialog`, `Collapsible`, `Accordion`, `Tabs`, `Tooltip`, `DataTable`) are directories, one file per part plus a namespace `index.ts` — see [`design-system.md`](design-system.md) for the full convention (AD-021).
