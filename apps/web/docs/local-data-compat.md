...
### `bf-hp-phases-view-v1` — additive fields (`pfr-web-ui`)

| Field | Default | Notes |
| --- | --- | --- |
| `farmPool` | `{}` (absent) | Farm Ranking rotation-pool override map, `Record<heroId, boolean>` (`AD-PFR-05`, `AD-PFRC-05`). A hero id absent from the map follows `HeroRecord.battleAllowed` — the map records only explicit local overrides. Bounded to the first 200 entries on load; a non-boolean entry or an empty-string key is dropped, siblings kept. An id naming a hero no longer in the roster is **kept in storage, ignored on use** — a read must not write (the `bf-hp-gear-scope-v1` precedent). **Never** written back to the save or to `bf-hp-heroes-v1` — this is estimation-local only. |
| `farmReturnBonus` | `'off'` (absent) | Farm Ranking Return Bonus estimate — `'off' \| 'on' \| 'vip'`, item B's `@bombfarm/domain/farm-rate` `ReturnBonusMode` reused verbatim as the persisted value (`ASM-C15`) rather than a local union. A stored value outside those three literals normalizes to `'off'`. |

Both fields stay **absent** on `defaultPhasesView()` and on every payload the player has never interacted with, so an untouched user's stored bytes are unchanged. `savePhasesView` writes the **whole** `PhasesViewState` on every call (`AD-PFRC-03`) — the slice's private `persistPhasesView` is the only composer, fixing a shipped bug where a one-field literal write (`savePhasesView({ phase })`) would have silently erased these fields the moment a second one existed.

### `bf-hp-account-v1` — additive field (`pfr-web-ui`, `OD-9`)

| Field | Default | Notes |
| --- | --- | --- |
| `maxPhase` | `null` | `account.max_phase` — the furthest phase this account has reached (`AD-PFR-02`). Mirrors the optional field item B (`pfr-domain-math`) added to its own storage shim. `null`/absent means no lock badges anywhere on the Farm Ranking board, and its unlocked-only filter renders disabled with a one-line reason rather than silently filtering to nothing. Clamped/rounded to an integer in `1..600` on load, the same way `context.phase` already is. **Written unconditionally by `applyAccountImport`** from `AccountImportData.maxPhase` — unlike every sibling field on this record, which is written only `if (data.field != null)`. Item B's field is required-and-total, so a payload carrying no `max_phase` source is an assertion that this account has no known max phase: a re-import must **clear** a stale value, not preserve it, or a lock badge would keep asserting progress the payload just contradicted. Both the file-import and API-refresh paths reach this (both funnel through `parseAccountPayload` → `mapAccountData`). |
...
| `glassCannon` / `tempoDobrado` / `abisso` / `abissoBase` / `critDmgMult` | MP5 F3, superseded MP5 F4 (`AD-089`) | The five keystone-derived `TreeState` fields. The 2026-08-13 game patch removed all five keystones; `@bombfarm/domain` stopped modelling them (MP5 F2), and MP5 F3 removed the Account panel's three Switch toggles that wrote them (`D24`'s wrong-not-missing risk — a control that still wrote a field the engine no longer honoured) and made `normalizeAccount`'s tree rebuild (`normalizeTree`, `account-shared.ts`) discard the five fields on load. **MP5 F4 supersedes that discard-and-keep behavior for these five fields** (`AD-089`; `MSG-21`): a stored planner account carrying any of them is now **dropped whole**, not discarded-and-kept. `dropStaleLocalAccount()` (`stale-account.ts`) reads the raw `bf-hp-account-v1` / `bf-hp-heroes-v1` (per-hero `tree` copy) / legacy `bf-pa-*` bytes as the first statement of `hydratePlannerStore()`, before `normalizeAccount` ever runs — trigger is field **presence**, not truthiness. On a hit, every `bf-hp-*` key is cleared and the app starts from its existing empty state; nothing is migrated forward (`storage-legacy-keystone-fields.test.ts`, rewritten for F4). The storage key stays `bf-hp-account-v1` (no `-v2` bump) — dropping the whole record needs no migrator. The `obsHit`/`obsCrit` row above is **unaffected**: discard-and-keep is still correct for those fields; this supersession is scoped to the five keystone fields only. |
