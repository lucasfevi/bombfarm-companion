...
| Field | Default | Notes |
| --- | --- | --- |
| `battleAllowed` | `true` | From save `battle_allowed`. Toggleable in the hero strip and roster picker. Disabled heroes (`false`) are excluded from roster respec recommendations; does not change DPS math. Refreshed from the save on re-import. |
