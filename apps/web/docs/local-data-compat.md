...
2. **All domain persistence goes through `src/shared/lib/storage.ts`.** New hero/account fields land on `HeroRecord` / `AccountShared` (or nested types), not new ad-hoc domain keys. **Exceptions:** (a) the gear-optimizer item inventory is account-scoped, replaced wholesale on every import, and never merged hero-by-hero — see [`bf-hp-inventory-v1`](#bf-hp-inventory-v1-account-scoped-collection) below; (b) Optimizer per-hero scope is a separate UI-pref map under [`bf-hp-gear-scope-v1`](#bf-hp-gear-scope-v1-optimizer-hero-scope) (key string kept for compat); (c) the Optimizer's last plan is a derived artifact written whole on its own cadence under [`bf-hp-team-plan-v1`](#bf-hp-team-plan-v1-optimizer-plan-envelope).
...
| `statRanges` | omit / `undefined` | The window each `birth` value was rolled inside, in planner units from save `stat_ranges`. Additive — missing until re-import, no migrator (rule 6). Per statistic and independently optional: a statistic whose window is missing, non-numeric or inverted is dropped and the rest are kept. **Absence never rejects anything**, unlike `birth`, where a partial roll rejects the whole save: these bounds compose no sheet math, so a save (or a live account read) carrying no `stat_ranges` at all must still import normally. |
| `runes` | `[]` | The timed rune buffs the hero carried when it was read, in planner units from save `runas` (`{axis, strengthPct, playSecondsLeft, rarity}`), already folded into `gearedOverride`. Additive — a record written before the importer read them loads with the empty list from `normalizeHeroRunes`, no migrator (rule 6). One malformed entry drops alone; absence never rejects anything. The draft carries the list untouched (only import writes it), so an autosave hands back what it loaded. See `docs/hero-runes.md` for how each figure treats a buff that expires. |
| `abilities` (levels) | copied verbatim | Ability ranks are stored **uncapped** — `normalizeHero` copies `raw.abilities` with no clamp, no key filter (`storage.ts`). The catalog's `AbilityDef.max` (20, W3) is a **UI/authoring bound** (stepper cap, `abilityPointBudget`), not a persistence bound: a record with a level above the current `max` (impossible from a save today, possible from hand-edited storage) loads and renders as-is, with the stepper's `+` disabled. Raising `max` in a catalog update is purely additive for persistence — no migrator needed (rule 6). |
...
Import confirm calls `replaceInventoryFromImport`, which clears any in-progress optimizer plan.

### `bf-hp-gear-scope-v1` (Optimizer hero scope)

**Stated exception to rule 2:** Optimizer Optimize / Donate / Leave-alone choices are account-wide UI prefs for the optimizer, not fields on `HeroRecord` / `AccountShared`. Load/save lives in [`team-plan-scope-storage.ts`](../src/shared/lib/team-plan-scope-storage.ts) (`TEAM_PLAN_SCOPE_KEY`). The **storage key string stays `bf-hp-gear-scope-v1`** for backward compatibility (module renamed gear-plan → team-plan; renaming the key would break existing public saves without a migrator). Payload shape is additive-stable: `Record<string, 'optimize' \| 'donate' \| 'leaveAlone'>`. Unknown hero ids are ignored on use; malformed values are dropped on load. Absent key → empty map → defaults from `battleAllowed` when the page builds scope.

### `bf-hp-team-plan-v1` (Optimizer plan envelope)

**Stated exception to rule 2:** the Optimizer's last plan is a derived, replaceable artifact — a whole plan per solve, written on its own cadence — and must not ride on the account record's autosave, which would rewrite every account field each time a solve lands and drag the plan along with every unrelated account edit. Load/save lives in [`team-plan-storage.ts`](../src/shared/lib/team-plan-storage.ts) (`TEAM_PLAN_KEY`). Payload shape: `{ version: 1, signature, objective, allowedChanges, ignoreFieldCrowding, targetPhase, plan }`, where `signature` is the input signature the plan was solved under and `plan` is the domain `TeamPlan` verbatim. **Whole-or-nothing load:** a stored value that is not a plain object, carries another `version`, lacks a string `signature`, fails any control's validator, or whose `plan` is missing any of `steps`, `forgeList`, `moveList`, `pointResets`, `perHero`, `currentDps`, `planDps`, `scoredPhaseSource` reads as no plan at all — never a partial one, and never a throw — and its key is removed at boot, the same way a plan solved under other controls is dropped (an envelope from before 2026-09-21 carrying the retired `'dps'` objective goes this way). **Restore rule:** at boot the plan is restored only when its four controls (`objective`, `allowedChanges`, `ignoreFieldCrowding`, `targetPhase` — the phase the plan is scored at under its objective, so a gate clear's gate) equal the booted store's; it is then current when `signature` matches the live input signature and stale otherwise. A plan solved under different controls is dropped and its key removed. The controls themselves are **not** persisted by this key — they boot to their defaults as before; the envelope only records what the plan was solved under. Written whole on every solve, removed on clear, never merged.
...
| `glassCannon` / `tempoDobrado` / `abisso` / `abissoBase` / `critDmgMult` | 2026-08-13 game patch | Five `TreeState` fields for skill-tree mechanics the 2026-08-13 patch removed from the game; `@bombfarm/domain` stopped modelling them and the Account panel's three Switch toggles that wrote them were removed. Records carrying them **load without error**, with the fields discarded on normalize — `normalizeAccount`'s tree rebuild (`normalizeTree`, `account-shared.ts`) is a fixed-field-list rebuild, so a stale key never spreads through. Same stated exception to rule 5 as the `obsHit`/`obsCrit` row above; the storage key stays `bf-hp-account-v1` (no `-v2` bump). |
