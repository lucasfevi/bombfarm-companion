...
| Field | Default | Notes |
| --- | --- | --- |
| Numeric totals (`danoTotal`, `critChance`, `critDmg`, `speed`, `energy`, `teamCoinPct`) | identity / `0` | From save `skills.totals` via `mapAccountData` → `applyAccountImport` / `hydrateAccount`. **Account UI is read-only** for these floats (`ReadonlyNum`) so display rounding cannot truncate full-precision store values — re-import to refresh. There are **no** per-field `setTree*` writers for them; only keystone toggles remain editable in the panel. |
