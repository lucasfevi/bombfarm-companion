...
2. **WHEN** renaming, moving, adding, or removing a panel or control that the explain text names (Account, House, Skill Tree, the Combat tab's team aura switches, Points, Effective stats, Farm page / farm phase, cycle/walk/mit/prop, …) **THEN** rewrite the explain (and guide steps that mirror it) to use the **current UI names** — do not leave retired panel names (e.g. Context, Gates, Math check, the Account page's Team buffs) or retired controls (Use as farm phase) or removed columns (e.g. Need%).
...
## Optimizer (`@bombfarm/domain/team-plan` + Optimizer page)

WHEN the Optimizer objective, duty weighting (`drainMult`), min-forge normalization (`effectiveUpgrade` / account `forgeFloor`), saturated vs under-saturated regime, final points-pass acceptance, or related UI copy changes, THEN update this doc and **both** `explainSections` entries **9 · Optimizer** / **9 · Otimizador** in `src/shared/i18n/namespaces/advice.ts`, and keep `explain-math-team-plan.test.ts` asserting EN + PT mention duty weighting and minimum forge / forja mínima in that section.
