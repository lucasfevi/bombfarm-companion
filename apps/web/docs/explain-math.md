...
## Team plan (`@bombfarm/domain/team-plan` + Team plan page)

WHEN the Team plan objective, duty weighting (`drainMult`), min-forge normalization (`effectiveUpgrade` / account `forgeFloor`), saturated vs under-saturated regime, final points-pass acceptance, or related UI copy changes, THEN update this doc and **both** `explainSections` entries **9 · Team plan** / **9 · Plano do time** in `src/shared/i18n/namespaces/advice.ts`, and keep `explain-math-team-plan.test.ts` asserting EN + PT mention duty weighting and minimum forge / forja mínima in that section.
