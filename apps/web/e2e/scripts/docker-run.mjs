 ...
 *
 * Usage:
 *   node e2e/scripts/docker-run.mjs test              # full suite (smoke — same as CI)
 *   node e2e/scripts/docker-run.mjs smoke             # the smoke project explicitly
...
const MODES = new Set(['test', 'smoke', 'perf', 'build-image', 'shell']);

function usage(exitCode = 0) {
  const text = `Usage: node e2e/scripts/docker-run.mjs <mode> [-- playwright-args…]

Modes:
  test         Full suite — the smoke specs, same as CI (there is no visual suite)
  smoke        The smoke project explicitly
