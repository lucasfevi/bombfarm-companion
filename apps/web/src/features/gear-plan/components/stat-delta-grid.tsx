...
export function StatDeltaGrid({
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_auto_auto] gap-x-3 gap-y-1 text-[12px]">
      ...
      <span className="text-right text-muted">{t.gearPlanColBefore}</span>
      <span className="text-right text-muted">{t.gearPlanColAfter}</span>
      <span className="text-right text-muted">{t.gearPlanColDelta}</span>
      {rows.map((row) => {
