...
function defaultWorkerFactory(): GearPlanWorkerLike {
  return new Worker(new URL('./worker/gear-plan.worker.ts', import.meta.url), {
    type: 'module',
  }) as unknown as GearPlanWorkerLike;
}
...
export function createGearPlanRunner(options?: {
  ...
  return {
    ...
    // test-only subscription hook
    subscribe(listener: () => void) {
      listeners.add(listener);
      return () => listeners.delete(listener);
    },
  } as GearPlanRunner & { subscribe(listener: () => void): () => void };
}

export function useGearPlanRunner(options?: { createWorker?: GearPlanWorkerFactory }) {
  const runnerRef = useRef<GearPlanRunner & { subscribe?(listener: () => void): () => void }>();
  if (!runnerRef.current) {
    runnerRef.current = createGearPlanRunner(options);
  }
  const [, bump] = useState(0);
  const subscribeRef = useRef(runnerRef.current.subscribe?.(() => bump((n) => n + 1)));
  if (!subscribeRef.current && runnerRef.current.subscribe) {
    subscribeRef.current = runnerRef.current.subscribe(() => bump((n) => n + 1));
  }
