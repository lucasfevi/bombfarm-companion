 ...
 */
export function MapCard({ lang, map }: { lang: Lang; map: ReplicaFrame['map'] }) {
  return (
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-baseline justify-between gap-3">
          <span className="flex min-w-0 flex-col gap-0.5">
            <span className="text-[20px] leading-none font-bold text-ink">
          ...
          <span className="text-[13px] font-bold whitespace-nowrap text-muted tabular-nums">
            #{map.phase}
          </span>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <ReplicaReading
            label={liveLabel('liveMapPropsLabel', lang)}
            value={`${formatNumber(map.propsAlive, lang, 0)}/${formatNumber(map.propsTotal, lang, 0)}`}
          />
        </div>
      </div>
