...
describe('team-plan i18n namespace', () => {
  it('exports matching EN and PT key sets', () => {
    expect(Object.keys(teamPlan.en).sort()).toEqual(Object.keys(teamPlan.pt).sort());
  });

  it('registers every namespace key on STRINGS in both languages', () => {
    for (const key of Object.keys(teamPlan.en)) {
  ...
  it('uses the sanctioned results header in both languages', () => {
    expect(STRINGS.en.teamPlanResultsHeader).toBe(SANCTIONED_HEADER.en);
    expect(STRINGS.pt.teamPlanResultsHeader).toBe(SANCTIONED_HEADER.pt);
  });
