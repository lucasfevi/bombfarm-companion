...
describe('front page metadata', () => {
  it('the sitemap lists seven routes, the front page first at full priority and the heroes page at its own address', () => {
    const entries = sitemap();

    expect(entries).toEqual([
      { url: `${SITE_URL}/`, changeFrequency: 'weekly', priority: 1 },
      { url: `${SITE_URL}/heroes`, changeFrequency: 'weekly', priority: 0.8 },
    ...
    expect(entries.map((entry) => entry.url)).not.toContain(`${SITE_URL}/planner`);
  });

  it("the front page carries the site title and the heroes page a title of its own, with its description unchanged", () => {
    const home = sectionMetadata('home');
    const heroes = sectionMetadata('heroes');
    ...
    expect(heroes.title).toBe('Heroes — Bomb Farm Companion');
    expect(heroes.description).toBe(HEROES_DESCRIPTION);
    expect(home.description).not.toBe(heroes.description);
  });

  it('the root path resolves to the home slug', () => {
    expect(sectionSlug('home')).toBe('home');
    expect(sectionSlug('heroes')).toBe('heroes');
  });
});
