...
## RULE: the download page draws the desktop Live screen — twice — keep both drawings current

`/download` shows a **replica** of the desktop app's Live tab
(`src/features/download/components/live/`) and, further down the page, a second drawing of the
desktop's **compact Live window** (`src/features/download/components/mini-live/`). Neither shares
code with the real screen and neither can: the boundaries forbid importing from `apps/desktop`, and
the desktop speaks its own bilingual copy layer ([`docs/i18n.md`](../../docs/i18n.md)). So they are
second implementations of screens that already exist, and second implementations rot quietly — the
page keeps rendering, no test goes red on its own, and one day it advertises a product that no
longer looks like that.

**If you change the desktop's Live screen or its compact window — a panel added, removed,
reordered, relabelled, or a figure's meaning changed — open the drawing in the same change and make
it match.** That is not optional politeness: the page is marketing, and a stale drawing is a false
...
| Compact-window pin | A compact-window component added, removed, or renamed | same file, `MINI_LIVE_COMPONENTS` |
...
the guard resolves against the desktop's copy module. They are exported in two groups:
`MIRRORED_BASE_KEYS`, asserted to render inside the full-size replica, and `MIRRORED_MINI_KEYS`,
asserted against the compact-window section. A key belonging to neither group has nothing drawing
it, which is what `src/tests/download-page.test.ts` exists to catch — do not fold the two groups
back into one to make an assertion pass.

When either component pin fails, the fix order matters: **update the drawing first, then re-pin in
the same commit.** Re-pinning alone turns the guard green over a drawing nobody looked at, which is
the one move that defeats the whole arrangement.
