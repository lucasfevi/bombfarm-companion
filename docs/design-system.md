...
**Gear slot card** (`GearSlotCard`, [`gear-slot-card.tsx`](../packages/hero/src/components/gear-slot-card.tsx); chrome `slotChromeClassName` in `@bombfarm/game-art`): one drawing for both hosts — slot name on top, the `xl` icon centered, the item's name with its forge `+N` in accent, tier (in its rarity colour) `·` level, and the slot's stat contributions beneath. The card stays **neutral** (`border-line`, solid once filled); the rarity reads from the item frame only. Empty slots show a dashed placeholder sharing `artFrameRadiusClass` and the `gearSlotEmptyTip` line. The web planner's [`slot-editor.tsx`](../apps/web/src/features/gear/components/slot-editor.tsx) hands its three selects and the clear button (pinned `absolute -top-1 -right-1`) in as the card's children; the desktop passes nothing and gets the read-only card. Gear compare clones draw the same card, with `changed` adding the inset accent ring.
