...
**Tooltip trigger nested inside another interactive control:** `Tooltip.Trigger` renders a `<button>` by default, which is invalid HTML nested inside another `<button>` (e.g. an `Accordion.Trigger` row). Swap the rendered tag via `render={<span />}` instead of `type="button"` — see `AbbreviatedNumber` (`apps/web/src/features/gear-plan/components/abbreviated-number.tsx`), which shows a `formatCompactNumber` value's exact figure on hover/focus from inside a Gear plan Accordion row. Pair with `tabIndex={-1}` (via a `disableFocus` prop) when the trigger sits inside an already-focusable ancestor, matching the icon-tooltip convention above.
