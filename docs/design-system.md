...
| `SearchSelectMultiple` | `@base-ui/react/combobox` (`multiple`) | `SelectMultiple`'s trigger (caller-rendered `renderValue`), optional header row and checkbox rows, plus `SearchSelect`'s search field and matcher inside the popup — for a tick list a player finds a row in by typing (the Heroes leaderboard's Columns control). No row cap: its lists are short | `search-select.recipe.ts` + `select.recipe.ts` |
...
| `Dialog` | `@base-ui/react/dialog` | compound `Root`/`Portal`/`Backdrop`/`Popup`/`Head`/`Title`/`Close`/`Body`/`Footer`; the popup ends with no bottom padding on purpose — a `Footer` rules a full-width line and pads its own actions, and a `Body` takes the height the head and footer leave and scrolls inside it (the consent modal is the reference shape) | `dialog.recipe.ts` (+ `panel-field.recipe.ts` for head/title) |
...
**Gear slot card** (`GearSlotCard`, [`gear-slot-card.tsx`](../packages/hero/src/components/gear-slot-card.tsx); chrome `slotChromeClassName` in `@bombfarm/game-art`): one drawing for both hosts — slot name on top, the `xl` icon centered, the item's name with its forge `+N` in accent, tier (in its rarity colour) `·` level, and the slot's stat contributions beneath. The card stays **neutral** (`border-line`, solid once filled); the rarity reads from the item frame only. Empty slots show a dashed placeholder sharing `artFrameRadiusClass` and the `gearSlotEmptyTip` line. The web planner's [`slot-editor.tsx`](../apps/web/src/features/gear/components/slot-editor.tsx) hands its three selects and the clear button (pinned `absolute -top-1 -right-1`) in as the card's children; the desktop passes nothing and gets the read-only card. Gear compare clones draw the same card, with `changed` adding the inset accent ring. The grid (`slotsGridClass`) is four across, centred, and eight across once its `@container` panel reaches 64rem — the two shapes in which every card sits beside slots of its own; never an auto-fit that folds 5+3.
