...
| --- | --- |
| `pnpm test` (the unscoped Vitest suite) | root `package.json`, through [`tools/with-heavy-slot.mjs`](../tools/with-heavy-slot.mjs) — two passes, the workspace projects and then the solver pass (`vitest.solver.config.ts`, the four long synchronous solver suites), each taking the slot in turn |
