...
**Status:** hard truth  
**Cursor stub:** [`.cursor/rules/validation.mdc`](../.cursor/rules/validation.mdc)
...
5. Spec-driven features: write/update `.specs/features/<slug>/validation.md` from the validator’s report (archive of that delivery).
6. Multi-feature orchestrated deliveries: also write a wave review under [`.specs/waves/`](../.specs/waves/) (see template) — still historical once filed.
...
Keep `pnpm test` as the fast Vitest unit gate — do not fold Playwright or Storybook into it. See [`e2e-visual.md`](e2e-visual.md) for the Docker e2e workflow (matches CI on any host), baselines, and **review-before-update**; [`design-system.md`](design-system.md) for the Storybook catalog.
