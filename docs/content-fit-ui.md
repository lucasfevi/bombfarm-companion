...
3. **Sibling alignment.** WHEN related controls share one stack column (e.g. House type `Select` + House level `Num`, or Skill Tree `ReadonlyNum` beside Team Buff `Num`) THEN they SHALL use the **same control width** so the value column lines up.
