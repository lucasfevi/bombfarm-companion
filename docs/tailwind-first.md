...
11. **Monorepo `@source` (hard):** Recipe / primitive class strings live in `packages/ui/src`. Every Tailwind CSS entry that styles UI which consumes `@bombfarm/ui` (today: `apps/web/src/app/globals.css`) **must** `@source` that tree. Omitting it drops package-only utilities from the bundle (common symptom: ability cards stack full-width because `abilGridClass`'s `auto-fill`/`240px` grid never generates). Do not rely on Next import-graph scanning for workspace packages.
12. **No unnecessary arbitrary values:** Prefer the named utility when it matches the theme (`tracking-[0.05em]` → `tracking-wider`, `min-w-[3.25rem]` → `min-w-13`, …). Enforced by `tailwindcss/no-unnecessary-arbitrary-value` (autofix) in `apps/web` + `packages/ui` ESLint. Keep intentional off-scale values as arbitrary (`tracking-[0.04em]`, `gap-[3px]`, `color-mix(…)`).
