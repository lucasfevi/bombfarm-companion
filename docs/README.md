...
| [farm-cadence-density.md](farm-cadence-density.md) | The measured farm-rate throughput error: the blast term does not decay with prop density — evidence, failed attempts, and the acceptance bar |
| [wire-vocabulary.md](wire-vocabulary.md) | Generated — the `/rotation` wire-to-domain vocabulary table; regenerate with `pnpm generate:wire-vocabulary` |
