...
2. **Wrap, don’t fork.** WHEN adopting a Base UI primitive THEN implement it under `src/shared/design-system/` with a `*.recipe.ts` + `cn()`, export from the barrel, and document it in [`design-system.md`](design-system.md).
